...
Api.prototype.uploadLanguageFiles = function(existingResources, callback) {
  ...
  for (i = 0, l = files.length; i < l; i++) {
    ...
    for (j = 0, m = existingResources.length; j < m; j++) {
      ...
      if (fileNameToSlug(file) === existingResource) {
  ...
  grunt.log.writeln('');
  grunt.log.writeln('---------------------------------------------');
  grunt.log.writeln('Source files to be overwritten on transifex: ', grunt.log.wordlist(existingFiles, colorOptions));
...
Api.prototype.uploadNewLanguageFiles = function(files) {
  ...
  for (i = 0, l = files.length; i < l; i++) {
    ...
    options = {
      ...
      json: {
        name: fileName,
        slug: fileNameToSlug(fileName), // same slug generation if you were to upload a file manually
        content: contents,
    ...
    request.post(options, function(error, response, body) {
      grunt.verbose.writeln('upload response: ', error, response, body);
